...
if (rangeToDisplay > 0) {
 ...
 levelUp() {
    ...
    /**
     * Aplica cura ao campeão, não excedendo o HP máximo.
     * @param {number} amount - Quantidade de HP a ser restaurada.
     */
    heal(amount) {
        // Garante que a cura é positiva
        const healAmount = Math.max(0, amount); 
        
        // Aplica a cura
        this.hp += healAmount;
        
        // Garante que o HP não excede o máximo
        if (this.hp > this.maxHp) {
            this.hp = this.maxHp;
        }

        // ⭐ Opcional: Adicionar um efeito visual de cura (TextPopEffect verde)
        // Isso assume que o champion tem acesso ao gameManager
        if (this.gameManager && this.gameManager.effects && this.gameManager.TextPopEffect) {
             this.gameManager.effects.push(new this.gameManager.TextPopEffect(
                this.getCenterX(), 
                this.getCenterY() - 10, 
                `+${Math.ceil(healAmount)}`, // Mostra o valor de cura arredondado
                '#32CD32', // Cor verde-limão
                500
            ));
        }
    }
...
export const TEAM_UPS = {
    psychicAttraction: {
       ...
       // ⚡ NOVO: CHAOS CONTROL (ESTILO MARVEL RIVALS)
    chaosControl: {
        name: 'Chaos Control',
        champions: ['wanda', 'emmafrost'],
        icon: 'ðŸ¤¯ðŸ',
        description: 'Feixe contínuo de energia do caos que puxa e atordoa inimigos',
        
        cooldown: 30000, // 30 segundos
        duration: 5000, // 5 segundos de feixe ativo
        range: 450, // Alcance máximo do feixe
        width: 60, // Largura do feixe
        
        damagePerTick: 25, // Dano por tick
        tickRate: 100, // 10 ticks por segundo
        
        pullSpeed: 150, // Velocidade de puxada
        pullDecay: 0.95, // Decaimento da força (mais longe = mais fraco)
        
        stunDuration: 1500, // Atordoa por 1.5s ao sair do feixe
        slowFactor: 0.6, // 60% mais lento dentro do feixe
        
        healPercent: 0.3, // Wanda e Emma recuperam 30% do dano causado
        
        // Visual
        beamColor: 'rgba(255, 0, 100, 0.8)',
        coreColor: 'rgba(255, 255, 255, 1)',
        particleColors: ['#ff0066', '#ff0099', '#cc00ff', '#9900ff']
    }
};

// ==============================================
// 🧠 TEAM UP: ATRAÇÃO PSIÔNICA (Emma + Wanda)
// ==============================================
export class PsychicAttractionTeamUpEffect {
    // Agora recebe a Wanda também
    constructor(emma, wanda, gameManager) { 
        this.emma = emma;
        this.wanda = wanda; // Nova referência
        this.gameManager = gameManager;
        
        // Configurações da Habilidade
        this.duration = 6000;      // 6 segundos
        this.range = 350;          // Raio de alcance maior
        this.pullSpeed = 70;       // Velocidade de atração
        
        // ⭐ LÓGICA DE DRENAGEM
        this.drainAmount = 10;     // Dano por tick
        this.drainInterval = 500;  // Tick a cada 0.5 segundos
        this.lastDrainTime = 0;    // Controle de tempo do tick
    ...
    update(deltaTime, enemies) {
        // Verifica se acabou (ou se Emma ou Wanda morreram)
        if (Date.now() > this.endTime || this.emma.hp <= 0 || this.wanda.hp <= 0) { 
            this.releaseEnemies();
        ...
        // 1. Drenagem de Vida (Tick de Dano/Cura)
        if (Date.now() > this.lastDrainTime + this.drainInterval) {
            this.lastDrainTime = Date.now();
            
            this.affectedEnemies.forEach(enemy => {
                if (enemy.hp > 0) {
                    // 1a. Aplica Dano ao Inimigo
                    const finalDrainDamage = this.drainAmount + (this.emma.level * 2); // Dano escala com nível
                    enemy.takeDamage(finalDrainDamage, this.emma);
                    
                    // 1b. ⭐ CURA A EMMA (LIFESTEAL)
                    this.emma.heal(finalDrainDamage); 
                    
                    // Efeito visual de Dano e Cura
                    this.gameManager.effects.push(new this.gameManager.TextPopEffect(
                        enemy.getCenterX(), enemy.getCenterY() - 10, `${finalDrainDamage}`, 'red', 500
                    ));
                    this.gameManager.effects.push(new this.gameManager.TextPopEffect(
                        this.emma.getCenterX(), this.emma.getCenterY() - 30, `+${finalDrainDamage}💜`, 'purple', 800
                    ));
                }
            });
        }

// 2. Captura e Puxa Inimigos (lógica de movimento)
        const emmaCenter = this.emma.getCenterX();
        const emmaCenterY = this.emma.getCenterY(); // Adicionado Y para clareza

        // O ponto de destino é agora o centro da Emma
        const destinationX = emmaCenter;
        const destinationY = emmaCenterY;
        
        enemies.forEach(enemy => {
            if (enemy.hp > 0 && !enemy.isSeduced && !enemy.isBoss) {
                const dist = Math.hypot(emmaCenter - enemy.getCenterX(), this.emma.getCenterY() - enemy.getCenterY());
                
                if (dist < this.range) {
                    enemy.isSeduced = true;
                    enemy.originalSpeed = enemy.vel; 
                    enemy.canAttack = false;         
                    this.affectedEnemies.push(enemy);
                    
                    this.gameManager.effects.push(new this.gameManager.TextPopEffect(
                        enemy.getCenterX(), enemy.getCenterY() - 30, '✨', 'magenta', 1000
                    ));
                }
            }
        });

        // 3. Atualiza movimento dos inimigos seduzidos (puxa para a Emma)
        for (let i = this.affectedEnemies.length - 1; i >= 0; i--) {
            const enemy = this.affectedEnemies[i];

            if (enemy.hp <= 0) {
                this.affectedEnemies.splice(i, 1);
                continue;
            }

            // ⭐ Mude o cálculo do ângulo para usar o destino (Emma)
            const angle = Math.atan2(
                destinationY - enemy.getCenterY(),
                destinationX - enemy.getCenterX()
            );

            const moveStep = this.pullSpeed * (deltaTime / 1000);
    ...
    draw(ctx) {
        // ... (Mantenha o draw() da resposta anterior, mas ajuste as coordenadas para o centro entre Emma e Wanda, se quiser) ...
        const cx = this.emma.getCenterX(); // Mantendo a aura centrada na Emma, mas puxando para o meio
        const cy = this.emma.getCenterY();

        ctx.save();

        // 1. Aura Roxa/Rosa (Team Up)
        // ... (código do gradiente de cor) ...
        const alpha = 0.2 + Math.sin(this.pulsePhase) * 0.1;
        const gradient = ctx.createRadialGradient(cx, cy, 0, cx, cy, this.range);
        gradient.addColorStop(0, `rgba(138, 43, 226, ${alpha})`); // BlueViolet
        gradient.addColorStop(1, 'rgba(255, 105, 180, 0)');      // HotPink
        ...
        // 2. Linhas de Conexão (Inimigo -> Meio das Heroínas)
        // ... (código das linhas tracejadas) ...
        ...
        this.affectedEnemies.forEach(enemy => {
            ctx.beginPath();
            ctx.moveTo(enemy.getCenterX(), enemy.getCenterY());
            ctx.lineTo((this.emma.getCenterX() + this.wanda.getCenterX()) / 2, (this.emma.getCenterY() + this.wanda.getCenterY()) / 2);
            ctx.stroke();
        });

        ctx.setLineDash([]); 
        
        // 4. Timer de Team Up em cima da Emma
        const timeLeft = Math.ceil((this.endTime - Date.now()) / 1000);
        const midX = this.emma.getCenterX(); // Ponto central agora é a Emma
        const midY = this.emma.getCenterY() - 30; // Ajusta a altura

        ctx.fillStyle = 'white';
        ctx.font = 'bold 18px Arial';
        ctx.textAlign = 'center';
        ctx.fillText(`🧠💜 TEAM UP: ${timeLeft}s`, midX, midY);
    ...
    releaseEnemies() {
        this.affectedEnemies.forEach(enemy => {
            if (enemy.hp > 0) {
                enemy.isSeduced = false;
                enemy.canAttack = true;
                enemy.vel = enemy.data.speed || 1; 
                
                this.gameManager.effects.push(new this.gameManager.TextPopEffect(
                    enemy.getCenterX(), enemy.getCenterY() - 30, '💔', 'gray', 800
                ));
            }
        });
        this.affectedEnemies = [];
...
// ==============================================
// ⚡ TEAM UP: CHAOS CONTROL (WANDA + EMMA) - ESTILO MARVEL RIVALS
// ==============================================

export class ChaosControlBeam {
    constructor(wanda, emma, gameManager) {
        this.wanda = wanda;
        this.emma = emma;
        this.gameManager = gameManager;
        this.data = TEAM_UPS.chaosControl;
        
        // Posição inicial (entre Wanda e Emma)
        this.originX = (wanda.getCenterX() + emma.getCenterX()) / 2;
        this.originY = (wanda.getCenterY() + emma.getCenterY()) / 2;
        
        // ⚡ AUTOLOCK: Busca inimigo mais próximo
        this.lockedTarget = this.findNearestEnemy();
        
        this.spawnTime = Date.now();
        this.endTime = this.spawnTime + this.data.duration;
        this.lastDamageTick = Date.now();
        
        // Visual de distorção da realidade
        this.distortionSegments = [];
        this.chaosParticles = [];
        this.realityTears = [];
        this.pulsePhase = 0;
        this.glitchOffset = 0;
        
        // Inicializa segmentos do feixe
        this.initializeBeamSegments();
        
        console.log('⚡ Chaos Control autolock:', this.lockedTarget);
    }
    
    findNearestEnemy() {
        const enemies = this.gameManager.enemies.filter(e => e.hp > 0);
        if (enemies.length === 0) return null;
        
        let nearest = enemies[0];
        let minDist = Math.hypot(
            this.originX - enemies[0].getCenterX(),
            this.originY - enemies[0].getCenterY()
        );
        
        for (const enemy of enemies) {
            const dist = Math.hypot(
                this.originX - enemy.getCenterX(),
                this.originY - enemy.getCenterY()
            );
    ...
    initializeBeamSegments() {
        // Cria segmentos "quebrados" do feixe
        for (let i = 0; i < 15; i++) {
            this.distortionSegments.push({
    ...
    update(deltaTime, enemies) {
        ...
        // Verifica expiração ou morte dos champions
        if (currentTime > this.endTime || this.wanda.hp <= 0 || this.emma.hp <= 0) {
            if (this.lockedTarget) {
                this.lockedTarget.isChaosLocked = false;
            }
            console.log('⚡ Chaos Control encerrado');
            return false;
        }
        
        // Atualiza origem (segue Wanda e Emma)
        this.originX = (this.wanda.getCenterX() + this.emma.getCenterX()) / 2;
        this.originY = (this.wanda.getCenterY() + this.emma.getCenterY()) / 2;
        
        // ⚡ AUTOLOCK: Se alvo morreu, busca novo
        if (!this.lockedTarget || this.lockedTarget.hp <= 0) {
            this.lockedTarget = this.findNearestEnemy();
            
            if (!this.lockedTarget) {
                console.log('⚡ Sem alvos disponíveis');
                return false;
            }
            
            console.log('⚡ Novo alvo travado:', this.lockedTarget.type);
        }
        
        // Marca alvo como travado
        this.lockedTarget.isChaosLocked = true;
        
        // Atualiza visual
        this.pulsePhase += deltaTime / 30;
        this.glitchOffset += deltaTime / 20;
        
        // Atualiza segmentos de distorção
        this.distortionSegments.forEach(seg => {
            seg.phase += seg.speed;
            seg.offset = Math.sin(seg.phase) * 30;
        });
        
        // Aplica efeitos
        if (currentTime - this.lastDamageTick >= this.data.tickRate) {
            this.applyBeamEffects();
    ...
    applyBeamEffects() {
        if (!this.lockedTarget || this.lockedTarget.hp <= 0) return;
        ...
        enemy.vel = enemy.originalSpeed * (1 - this.data.slowFactor);
        
        // Calcula força de puxada (mais forte quanto mais próximo)
        const distToOrigin = Math.hypot(
            enemy.getCenterX() - this.originX,
            enemy.getCenterY() - this.originY
        );
        
        const pullStrength = this.data.pullSpeed * Math.pow(this.data.pullDecay, distToOrigin / 50);
        ...
        enemy.x += Math.cos(pullAngle) * pullStrength * 0.016;
        enemy.y += Math.sin(pullAngle) * pullStrength * 0.016;
        
        // Aplica dano
        const damage = this.data.damagePerTick;
        enemy.takeDamage(damage, this.wanda);
        
        // Cura Wanda e Emma
        const healAmount = damage * this.data.healPercent;
        
        if (this.wanda.hp < this.wanda.maxHp) {
            this.wanda.hp = Math.min(this.wanda.maxHp, this.wanda.hp + healAmount);
        }
        
        if (this.emma.hp < this.emma.maxHp) {
            this.emma.hp = Math.min(this.emma.maxHp, this.emma.hp + healAmount);
        }
        ...
        // Cria "rasgos na realidade" ao redor do alvo
        this.createRealityTears(enemy);
    }
    
    createRealityTears(enemy) {
        for (let i = 0; i < 3; i++) {
            const angle = Math.random() * Math.PI * 2;
            const dist = 20 + Math.random() * 30;
            
            this.realityTears.push({
                x: enemy.getCenterX() + Math.cos(angle) * dist,
                y: enemy.getCenterY() + Math.sin(angle) * dist,
                width: 2 + Math.random() * 3,
                height: 15 + Math.random() * 20,
                rotation: angle + Math.PI / 2,
                life: 1,
                color: this.data.particleColors[Math.floor(Math.random() * this.data.particleColors.length)]
            });
        }
    }
    ...
    updateParticles(deltaTime) {
        ...
        // Atualiza rasgos na realidade
        this.realityTears = this.realityTears.filter(tear => tear.life > 0);
        this.realityTears.forEach(tear => {
            tear.life -= 0.03;
            tear.width *= 0.98;
        });
        
        // Cria partículas de caos ao longo do feixe
        if (Math.random() < 0.5) {
            const t = Math.random();
            const px = this.originX + (this.lockedTarget.getCenterX() - this.originX) * t;
            const py = this.originY + (this.lockedTarget.getCenterY() - this.originY) * t;
            
            this.chaosParticles.push({
        ...
        // Atualiza partículas
        this.chaosParticles = this.chaosParticles.filter(p => p.life > 0);
    ...
    draw(ctx) {
        ...
        const targetX = this.lockedTarget.getCenterX();
        const targetY = this.lockedTarget.getCenterY();
        
        ctx.save();
        
        // ===============================
        // FEIXE "QUEBRADO" COM DISTORÇÃO
        // ===============================
        
        const segments = 15;
        const baseAngle = Math.atan2(targetY - this.originY, targetX - this.originX);
        
        // Camada 1: Núcleo branco pulsante
        ctx.strokeStyle = 'rgba(255, 255, 255, 1)';
        ctx.lineWidth = 6 + Math.sin(this.pulsePhase * 2) * 2;
        ...
        for (let i = 0; i <= segments; i++) {
            const t = i / segments;
            const baseX = this.originX + (targetX - this.originX) * t;
            const baseY = this.originY + (targetY - this.originY) * t;
            
            // Distorção baseada no segmento
            const seg = this.distortionSegments[i % this.distortionSegments.length];
            const perpAngle = baseAngle + Math.PI / 2;
            const distortX = baseX + Math.cos(perpAngle) * seg.offset;
            const distortY = baseY + Math.sin(perpAngle) * seg.offset;
            
            if (i === 0) {
                ctx.moveTo(distortX, distortY);
            } else {
                ctx.lineTo(distortX, distortY);
            }
        }
        
        ctx.stroke();
        
        // Camada 2: Feixe vermelho/rosa/roxo principal (MÚLTIPLAS CORES)
        const colors = [
            'rgba(255, 0, 100, 0.9)',
            'rgba(255, 0, 150, 0.8)',
            'rgba(200, 0, 150, 0.9)',
        ...
        for (let layer = 0; layer < 4; layer++) {
            ...
            ctx.lineWidth = 20 - layer * 4 + Math.sin(this.pulsePhase + layer) * 3;
            ...
            for (let i = 0; i <= segments; i++) {
                ...
                const seg = this.distortionSegments[i % this.distortionSegments.length];
                const perpAngle = baseAngle + Math.PI / 2;
                const distortX = baseX + Math.cos(perpAngle) * seg.offset * (1 + layer * 0.2);
                const distortY = baseY + Math.sin(perpAngle) * seg.offset * (1 + layer * 0.2);
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const glitchAngle = baseAngle + (Math.random() - 0.5) * Math.PI / 2;
            const glitchDist = 30 + Math.random() * 40;
            const glitchX = baseX + Math.cos(glitchAngle) * glitchDist;
            const glitchY = baseY + Math.sin(glitchAngle) * glitchDist;
            
            const glitchAlpha = 0.3 + Math.random() * 0.4;
            ctx.strokeStyle = `rgba(255, ${Math.floor(Math.random() * 100)}, 200, ${glitchAlpha})`;
        ...
        // ===============================
        // RASGOS NA REALIDADE (TEARS)
        // ===============================
        this.realityTears.forEach(tear => {
            ctx.save();
            ctx.translate(tear.x, tear.y);
            ctx.rotate(tear.rotation);
            
            ctx.globalAlpha = tear.life * 0.9;
            
            const tearGradient = ctx.createLinearGradient(-tear.width / 2, 0, tear.width / 2, 0);
            tearGradient.addColorStop(0, tear.color.replace(/[\d.]+\)/, '0)'));
            tearGradient.addColorStop(0.5, tear.color);
            tearGradient.addColorStop(1, tear.color.replace(/[\d.]+\)/, '0)'));
        ...
        this.chaosParticles.forEach(p => {
            ...
            particleGradient.addColorStop(0.7, p.color);
            particleGradient.addColorStop(1, p.color.replace(/[\d.]+\)/, '0)'));
            
            ctx.fillStyle = particleGradient;
            ctx.shadowColor = p.color;
            ctx.shadowBlur = 12;
            
            // Formato de "diamante distorcido"
            ctx.beginPath();
            ctx.moveTo(0, -p.size);
        ...
        // ===============================
        // AURA NO ALVO (TRAVADO)
        // ===============================
        const lockPulse = 1 + Math.sin(this.pulsePhase * 3) * 0.3;
        const lockGradient = ctx.createRadialGradient(
            targetX, targetY, 0,
            targetX, targetY, 50 * lockPulse
        );
        lockGradient.addColorStop(0, 'rgba(255, 0, 100, 0.6)');
        lockGradient.addColorStop(0.5, 'rgba(200, 0, 150, 0.4)');
        lockGradient.addColorStop(1, 'rgba(150, 0, 200, 0)');
        
        ctx.fillStyle = lockGradient;
        ctx.beginPath();
        ctx.arc(targetX, targetY, 50 * lockPulse, 0, Math.PI * 2);
        ctx.fill();
        
        // Anel de trava
        ctx.strokeStyle = `rgba(255, 0, 100, ${0.8 * lockPulse})`;
        ctx.lineWidth = 3;
        ctx.shadowColor = '#ff0066';
        ctx.shadowBlur = 15;
        ctx.beginPath();
        ctx.arc(targetX, targetY, 40 * lockPulse, 0, Math.PI * 2);
        ctx.stroke();
        ctx.shadowBlur = 0;
        ...
        const progress = timeLeft / this.data.duration;
        
        const barWidth = 120;
        const barHeight = 6;
        const barX = this.originX - barWidth / 2;
        const barY = this.originY - 50;
        
        ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
        ctx.fillRect(barX, barY, barWidth, barHeight);
        
        const barGradient = ctx.createLinearGradient(barX, 0, barX + barWidth * progress, 0);
        barGradient.addColorStop(0, 'rgba(255, 0, 100, 1)');
        barGradient.addColorStop(0.5, 'rgba(200, 0, 150, 1)');
        ...
        ctx.shadowColor = '#ff0066';
        ctx.shadowBlur = 12;
        ctx.fillText('⚡💜 CHAOS CONTROL', this.originX, barY - 5);
        ctx.shadowBlur = 0;
        
        // Indicador de alvo travado
        ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
        ctx.font = 'bold 11px Arial';
        ctx.fillText(`ðŸŽ¯ TRAVADO: ${this.lockedTarget.type.toUpperCase()}`, targetX, targetY - 60);
...
export class Thor extends Champion {
    constructor(type, x, y, id, gameManagerInstance) {
        ...
        this.isMjolnirActive = false; // Flag para controlar se o Mjolnir está fora (para o arremesso)
         ...
         // âœ… NOVO: Contadores para sons especiais
        this.lightningKillCount = 0; // Conta kills com raio
        this.mjolnirThrowCount = 0;  // Conta arremessos de Mjolnir
    }
...
// Dentro da classe EmmaFrost em champions.js

    activateAbility(abilityNumber) {
        const championData = Champion.championData.emmafrost;
        
        // 1. Troca de Modo (Mantenha igual)
        if (abilityNumber === 1 && this.abilities.toggleForm.cooldown <= 0) {
            ...
            if (this.gameManager.EmmaFormChangeEffect) {
                this.gameManager.effects.push(new this.gameManager.EmmaFormChangeEffect(this.getCenterX(), this.getCenterY(), 800, this.isDiamondForm));
            }
            this.gameManager.showUI(`Emma Frost: Modo ${this.isDiamondForm ? 'Diamante' : 'Psíquico'}`, 'info');
        }
        
        // 2. Impacto de Diamante (Mantenha igual)
        else if (abilityNumber === 2 && this.abilities.diamondImpact.cooldown <= 0 && this.isDiamondForm) {
            // ... (seu código existente do Impacto de Diamante) ...
        ...
        // 3. Rajada Mental (Mantenha igual)
            ...
            // ... (seu código existente da Rajada Mental) ...
            this.gameManager.showUI('Emma Frost: Rajada Mental!', 'success');
        }

// ⭐ 4. TEAM UP: ATRAÇÃO PSIÔNICA (Exige Wanda e inclui Drenagem)
    else if (abilityNumber === 4) {
        
        // 1. Verifica Cooldown e Modo
        if (this.teamUpCooldown > 0) {
        ...
        if (this.isDiamondForm) {
            this.gameManager.showUI('Preciso estar em forma Psíquica para Team Up!', 'warning');
            return;
        }
        ...
        if (!wanda) {
            this.gameManager.showUI('Emma Frost: Preciso da Wanda para o Team Up Psiônico! 💜', 'warning');
            return;
        }
        
        // 3. Instancia o Team Up
        if (!this.gameManager.psychicAttractionCones) {
            this.gameManager.psychicAttractionCones = [];
        }
        
        // Instancia a nova classe de efeito, passando Emma e Wanda
        const teamUp = new PsychicAttractionTeamUpEffect(this, wanda, this.gameManager);
        this.gameManager.psychicAttractionCones.push(teamUp);
        
        // Aplica Cooldown
        this.teamUpCooldown = 30000; // 30 segundos de cooldown
        
        this.gameManager.effects.push(new this.gameManager.TextPopEffect(
            (this.getCenterX() + wanda.getCenterX()) / 2,
            (this.getCenterY() + wanda.getCenterY()) / 2 - 80,
            'TEAM UP!',
            'purple',
            1500
        ));

        this.gameManager.showUI('Emma + Wanda: ATRAÇÃO PSIÔNICA ativada! 🧠💜', 'ultimate');
    }
}
...
activateAbility(abilityNumber) {
    ...
    // Habilidade 1: Postura Defensiva
    if (abilityNumber === 1 && this.abilities.defensiveStance.cooldown <= 0) {
        this.isDefensiveStanceActive = true;
        this.defensiveStanceEndTime = Date.now() + data.defensiveStanceDuration;
        
        // CORREÇÃO: Usa 'data' em vez de Champion.captainamerica
        this.applyBuff('damageReduction', data.damageReductionFactor, data.defensiveStanceDuration);

        this.hp = Math.min(this.maxHp, this.hp + data.defensiveStanceShieldAmount);
        
        this.gameManager.effects.push(new this.gameManager.DefensiveStanceEffect(
            this.getCenterX(), 
            this.getCenterY(), 
            data.defensiveStanceDuration, 
            this.width * 1.2
        ));
        
        this.abilities.defensiveStance.cooldown = data.defensiveStanceCooldown;
    ...
    // Habilidade 3: Grito de Liderança
        ...
        this.gameManager.champions.forEach(ally => {
            ...
            if (dist < data.leadershipCryRadius) {
                // CORREÇÃO AQUI TAMBÉM: Usar 'data' em vez de Champion.captainamerica
                ally.applyBuff('damageBoost', data.leadershipCryDamageBoost, data.leadershipCryDuration);
                ally.applyBuff('hpRegen', data.leadershipCryHpRegen, data.leadershipCryDuration, false); 
                
                this.gameManager.effects.push(new this.gameManager.USAgentCombatCallEffect(
                    ally.getCenterX(), 
                    ally.getCenterY(), 
                    data.leadershipCryDuration, 
                    'blue'
                ));
            }
        });
        this.abilities.leadershipCry.cooldown = data.leadershipCryCooldown;
...
export class Wanda extends Champion {
    constructor(type, x, y, id, gameManagerInstance) {
        ...
        // Inicializa cooldown do Team Up
        this.teamUpCooldown = 0; // Adicione ou confirme que isso existe
    }
    ...
    // ⭐ 2. Reality Erasure (Ataque Padrão - Team Up não afeta o ataque base)
    attack(enemies, projectiles, effects) {
...
/*         if (this.attackCooldown <= 0) {
            this.attackCooldown = Champion.championData.wanda.attackCooldown;
            
            const target = this.findNearestEnemy(this.gameManager.enemies);
            
            if (target) {
                const angle = Math.atan2(target.getCenterY() - this.getCenterY(), target.getCenterX() - this.getCenterX());
                
                // ⭐ PROJÉTIL Reality Erasure (Laser/Feixe rápido)
                this.gameManager.projectiles.push(new this.gameManager.RealityErasureProjectile(
                    this.getCenterX(), 
                    this.getCenterY(), 
                    target.getCenterX(), 
                    target.getCenterY(), 
                    Champion.championData.wanda.realityErasureSpeed, 
                    this.getDamage() * Champion.championData.wanda.realityErasureDamageMultiplier, // Usa o dano base da Wanda
                    this.id, 
                    this.gameManager
                ));
            }
    ...
    activateAbility(abilityNumber) {
        ...
        // ⚡ HABILIDADE 4: CHAOS CONTROL (TEAM UP COM EMMA)
        else if (abilityNumber === 4) {
            console.log('⚡ Tentando ativar Chaos Control...');
            
            const emma = this.gameManager.champions.find(c => c.type === 'emmafrost' && c.hp > 0);
            
            if (!emma) {
                this.gameManager.showUI('Wanda: Preciso de Emma! 💜', 'warning');
                return;
            }
            
            if (this.teamUpCooldown > 0) {
                const cdLeft = (this.teamUpCooldown / 1000).toFixed(1);
                this.gameManager.showUI(`Chaos Control: ${cdLeft}s`, 'warning');
                return;
            }
            
            // ⚡ AUTOLOCK: Não precisa de ângulo!
            const beam = new ChaosControlBeam(this, emma, this.gameManager);
            
            if (!this.gameManager.chaosControlBeams) {
                this.gameManager.chaosControlBeams = [];
            }
            this.gameManager.chaosControlBeams.push(beam);
            
            this.teamUpCooldown = TEAM_UPS.chaosControl.cooldown;
            
            // ✨ EFEITOS VISUAIS
            const centerX = (this.getCenterX() + emma.getCenterX()) / 2;
            const centerY = (this.getCenterY() + emma.getCenterY()) / 2;
            
            this.gameManager.effects.push(new this.gameManager.RedHulkExplosionEffect(
                centerX, centerY, 150, 1000, 'rgba(255, 0, 100, 0.9)'
            ));
            
            for (let i = 0; i < 5; i++) {
                setTimeout(() => {
                    this.gameManager.effects.push(new this.gameManager.USAgentShockwaveEffect(
                        centerX, centerY, 100 + i * 40, 600
                    ));
                }, i * 150);
            }
            
            this.gameManager.showUI('Wanda + Emma: CHAOS CONTROL! ⚡💜', 'ultimate');
        }
...
// ============================================
export class JeanGrey extends Champion {
constructor(type, x, y, id, gameManagerInstance) {
    super(type, x, y, id, gameManagerInstance);
    
    // Sistema de Faíscas
    this.sparkSystem = new Map();
    this.lastSparkExplosionTime = new Map();
    
    // 🔥 NOVO: Estado do Ressurgimento
    this.isPhoenixRebirthing = false;
    this.phoenixRebirthStartTime = 0;
    this.phoenixRebirthLandTime = 0;
    this.phoenixRebirthTargetX = 0;
    this.phoenixRebirthTargetY = 0;
    this.phoenixRebirthOriginalX = 0;
    this.phoenixRebirthOriginalY = 0;
    this.phoenixRebirthStoredHp = 0;

     // ⭐ NOVO: SISTEMA DE PODERES PSÍQUICOS ALTERNADOS
        this.lastPsychicPowerTime = 0;
        this.psychicPowerInterval = 3000; // 3 segundos
        this.nextPsychicPower = 'wave'; // Alterna entre 'wave' e 'vortex'
    
    
    // Habilidades
    this.abilities.telekineticBarrage = { 
        cooldown: 0, 
        cooldownBase: Champion.championData.jeangrey.telekineticBarrageCooldown 
    };
    
    this.abilities.telepathicIllusion = { 
        cooldown: 0, 
        cooldownBase: Champion.championData.jeangrey.telepathicIllusionCooldown 
    };
    
    this.abilities.phoenixRebirth = { 
        cooldown: 0, 
        cooldownBase: Champion.championData.jeangrey.phoenixRebirthCooldown 
    };
}
    ...
    update(deltaTime, enemies, champions, projectiles, effects) {
        super.update(deltaTime, enemies, champions, projectiles, effects);
         ...
         // ===== PODERES PSÍQUICOS AUTOMÁTICOS (A CADA 3s) =====
        if (Date.now() - this.lastPsychicPowerTime >= this.psychicPowerInterval) {
            this.usePsychicPower(enemies, effects);
            this.lastPsychicPowerTime = Date.now();
        }
        
        
        // Limpa Faíscas de inimigos mortos
        for (const [enemyId, count] of this.sparkSystem.entries()) {
            const enemy = enemies.find(e => e.id === enemyId);
            if (!enemy || enemy.hp <= 0) {
                this.sparkSystem.delete(enemyId);
                this.lastSparkExplosionTime.delete(enemyId);
            }
        }
    }

// Em champions.js - JeanGrey.usePsychicPower()

    usePsychicPower(enemies, effects) {
        // CORREÇÃO: A chave correta é 'jeangrey' (sem underscore)
        const data = Champion.championData.jeangrey;
        
        if (!data) {
            console.error("Dados da Jean Grey não encontrados!");
            return;
        }
        
        if (this.nextPsychicPower === 'wave') {
            // ===== ONDA PSÍQUICA (ROSA ELEGANTE) =====
            
            // Cria efeito visual
            if (this.gameManager.PsychicWaveEffect) {
                this.gameManager.effects.push(new this.gameManager.PsychicWaveEffect(
                    this.getCenterX(),
                    this.getCenterY(),
                    data.psychicWaveRadius,
                    800
                ));
            }
            ...
            this.gameManager.enemies.forEach(enemy => {
                ...
                if (dist < data.psychicWaveRadius) {
                    // Dano
                    enemy.takeDamage(data.psychicWaveDamage, this);
                    
                    // 40% chance de atordoar
                    if (Math.random() < data.psychicWaveStunChance) {
                        enemy.applyStun(data.psychicWaveStunDuration);
                        
                        if (this.gameManager.StunEffect) {
                            this.gameManager.effects.push(new this.gameManager.StunEffect(
                                enemy.getCenterX(),
                                enemy.getCenterY(),
                                data.psychicWaveStunDuration
                            ));
                        }
                    }
                    ...
                    enemy.x += Math.cos(pushAngle) * data.psychicWavePushForce;
                    enemy.y += Math.sin(pushAngle) * data.psychicWavePushForce;
                }
            });
            
            // Próximo poder será vórtice
            this.nextPsychicPower = 'vortex';
            
        } else {
            // ===== VÓRTICE DE IMPACTO (AZUL BRUTAL) =====
            
            // Encontra 2 inimigos: o mais próximo (vítima) e um distante (alvo)
            let victim = null;
            let target = null;
            let minDist = Infinity;
            let maxDist = 0;
            ...
            this.gameManager.enemies.forEach(enemy => {
                ...
                if (dist < minDist && dist < data.vortexRange) {
                    minDist = dist;
                    victim = enemy;
                }
            });
            
            // Se encontrou vítima, procura alvo distante
            if (victim) {
                this.gameManager.enemies.forEach(enemy => {
                    if (enemy.id !== victim.id) {
                        const distToVictim = Math.hypot(
                            victim.getCenterX() - enemy.getCenterX(),
                            victim.getCenterY() - enemy.getCenterY()
                        );
                        
                        if (distToVictim > maxDist) {
                            maxDist = distToVictim;
                            target = enemy;
                        }
                    }
                });
                
                // Se encontrou ambos, executa o vórtice
                if (target) {
                    // Efeito visual de sucção → trajetória → impacto
                    if (this.gameManager.VortexImpactEffect) {
                        this.gameManager.effects.push(new this.gameManager.VortexImpactEffect(
                            victim.getCenterX(),
                            victim.getCenterY(),
                            target.getCenterX(),
                            target.getCenterY(),
                    ...
                    // Após 0.7s, move a vítima e aplica dano
                    setTimeout(() => {
                        if (victim.hp > 0 && target.hp > 0) {
                            // Move vítima para o alvo
                            victim.x = target.x;
                            victim.y = target.y;
                            
                            // Dano na vítima
                            victim.takeDamage(data.vortexDamage, this);
                            
                            // Dano no alvo
                            target.takeDamage(data.vortexDamage, this);
                            
                            // Atordoa ambos
                            victim.applyStun(data.vortexStunDuration);
                            target.applyStun(data.vortexStunDuration);
                            
                            if (this.gameManager.StunEffect) {
                                this.gameManager.effects.push(new this.gameManager.StunEffect(
                                    target.getCenterX(),
                                    target.getCenterY(),
                                    data.vortexStunDuration
                                ));
                            }
                        }
                    }, 700);
            ...
            // Próximo poder será onda
            this.nextPsychicPower = 'wave';
        }
    }
    ...
    attack(enemies, projectiles, effects) {
        if (this.lastAttackTime <= 0) {
            ...
            if (target) {
                ...
                // 🔥 APLICA DANO E FAÍSCA
                target.takeDamage(finalDamage, this);
                this.addSpark(target, enemies, effects);
                
                // 🔥 EFEITO VISUAL - CHAMAS CÓSMICAS
                effects.push(new this.gameManager.PhoenixFlameEffect(
                    this.getCenterX(),
                    this.getCenterY(),
                    target.getCenterX(),
                    target.getCenterY(),
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    effects.push(new this.gameManager.AuraFireParticleEffect(
                        target.getCenterX() + Math.cos(angle) * 15,
                        target.getCenterY() + Math.sin(angle) * 15,
                        12,
                        'orange',
                        400
                    ));
    ...
    // 🔥 SISTEMA DE FAÍSCAS
    addSpark(enemy, enemies, effects) {
        const data = Champion.championData.jeangrey;
        const currentSparks = this.sparkSystem.get(enemy.id) || 0;
        const newSparks = currentSparks + 1;
        
        this.sparkSystem.set(enemy.id, newSparks);
        enemy.phoenixSparks = newSparks; // Para o visual no enemy.draw()
        
        // Efeito visual da Faísca
        effects.push(new this.gameManager.PhoenixSparkEffect(
            enemy.getCenterX(),
            enemy.getCenterY(),
            newSparks,
            600
        ...
        // 🔥 EXPLOSÃO AO ATINGIR 3 FAÍSCAS
        if (newSparks >= data.sparksToExplode) {
            const now = Date.now();
            const lastExplosion = this.lastSparkExplosionTime.get(enemy.id) || 0;
            
            // Previne explosões múltiplas em curto período
            if (now - lastExplosion > data.explosionCooldown) {
                this.triggerSparkExplosion(enemy, enemies, effects);
                this.sparkSystem.set(enemy.id, 0); // Reset
                this.lastSparkExplosionTime.set(enemy.id, now);
            }
        }
    }

    // 🔥 EXPLOSÃO DE FAÍSCAS
    triggerSparkExplosion(epicenter, enemies, effects) {
        const data = Champion.championData.jeangrey;
        const explosionDamage = data.explosionDamage * (1 + this.damageBoostBuff);
        
        // 🔥 EFEITO VISUAL ÉPICO
        effects.push(new this.gameManager.PhoenixExplosionEffect(
            epicenter.getCenterX(),
            epicenter.getCenterY(),
        ...
        for (let w = 1; w <= 3; w++) {
            setTimeout(() => {
                effects.push(new this.gameManager.USAgentShockwaveEffect(
                    epicenter.getCenterX(),
                    epicenter.getCenterY(),
                    data.explosionRadius * (0.7 + w * 0.15),
                    400
                ));
            }, w * 100);
        }
        ...
        enemies.forEach(enemy => {
            const dist = Math.hypot(
                epicenter.getCenterX() - enemy.getCenterX(),
                epicenter.getCenterY() - enemy.getCenterY()
            );
            
            if (dist < data.explosionRadius) {
                enemy.takeDamage(explosionDamage, this);
                enemiesHit++;
                
                // Aplica 1 Faísca aos inimigos ao redor (mas não explode em cascata)
                if (enemy.id !== epicenter.id) {
                    const currentSparks = this.sparkSystem.get(enemy.id) || 0;
                    this.sparkSystem.set(enemy.id, Math.min(currentSparks + 1, 2)); // Max 2 para evitar cascata
                ...
                effects.push(new this.gameManager.TextPopEffect(
                    enemy.getCenterX(),
                    enemy.getCenterY() - 20,
                    `🔥 ${explosionDamage.toFixed(0)}`,
                    'orange',
                    800
                ));
            }
        ...
        if (enemiesHit > 1) {
            this.gameManager.showUI(`Jean Grey: ${enemiesHit} inimigos incinerados!`, 'ultimate');
        }
    }

    // 🔥 VISUAL CUSTOMIZADO - AURA DE FOGO
    draw(ctx, isSelected, mjolnirImage, capShieldImage, usagentShieldImage, wandaIllusionImage, displayRangeOverride = null) {
        // Draw base primeiro
        super.draw(ctx, isSelected, mjolnirImage, capShieldImage, usagentShieldImage, wandaIllusionImage, displayRangeOverride);
        ...
        // ===============================
        const auraSize = 45 + Math.sin(time * 3) * 8;
        const auraGradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, auraSize);
        auraGradient.addColorStop(0, 'rgba(255, 100, 0, 0)');
        auraGradient.addColorStop(0.4, 'rgba(255, 150, 0, 0.4)');
        auraGradient.addColorStop(0.7, 'rgba(255, 69, 0, 0.3)');
        ...
        // ===============================
        for (let i = 0; i < 8; i++) {
            const flameAngle = (Math.PI * 2 / 8) * i + time * 1.5;
            const flameRadius = 35 + Math.sin(time * 4 + i) * 5;
            const fx = centerX + Math.cos(flameAngle) * flameRadius;
            const fy = centerY + Math.sin(flameAngle) * flameRadius;
            
            const flameSize = 8 + Math.sin(time * 5 + i) * 3;
            const flameGradient = ctx.createRadialGradient(fx, fy, 0, fx, fy, flameSize);
            flameGradient.addColorStop(0, `rgba(255, 255, 100, ${0.8 + Math.sin(time * 6 + i) * 0.2})`);
            flameGradient.addColorStop(0.5, `rgba(255, 150, 0, ${0.6 + Math.sin(time * 6 + i) * 0.2})`);
        ...
        // ===============================
        // FÊNIX SÍMBOLO NO TOPO
        // ===============================
        const symbolAlpha = 0.7 + Math.sin(time * 3) * 0.3;
        ctx.fillStyle = `rgba(255, 100, 0, ${symbolAlpha})`;
...
activateAbility(abilityNumber) {
    ...
    // ===============================
    // HABILIDADE 1: RAJADA DE TELECINESE
    // ===============================
    if (abilityNumber === 1 && this.abilities.telekineticBarrage.cooldown <= 0) {
        // Ativa modo de seleção de área
        this.gameManager.isSelectingTelekineticBarrage = true;
        this.gameManager.telekineticBarrageOwner = this;
        
        this.gameManager.showUI('Jean Grey: Clique para marcar a área da Rajada Telecinética!', 'info');
        
        this.abilities.telekineticBarrage.cooldown = data.telekineticBarrageCooldown;
    }
    
    // ===============================
    // HABILIDADE 2: ILUSÃO TELEPÁTICA (MELHORADA)
    // ===============================
    else if (abilityNumber === 2 && this.abilities.telepathicIllusion.cooldown <= 0) {
        const moveDistance = data.illusionMoveDistance;
        
        // Salva posição atual (para o clone)
        const cloneX = this.getCenterX();
        const cloneY = this.getCenterY();
        
        // Move Jean Grey para trás (esquerda)
        this.x -= moveDistance;
        ...
        // 🔥 NOVO: Busca inimigo mais forte no alcance
        let strongestEnemy = null;
        let maxHp = 0;
        
        this.gameManager.enemies.forEach(enemy => {
            const dist = Math.hypot(cloneX - enemy.getCenterX(), cloneY - enemy.getCenterY());
            if (dist < data.illusionSearchRadius && enemy.hp > maxHp) {
                maxHp = enemy.hp;
                strongestEnemy = enemy;
            }
        });
        
        // Se não encontrou ninguém no alcance, busca o mais forte geral
        if (!strongestEnemy && this.gameManager.enemies.length > 0) {
            strongestEnemy = this.gameManager.enemies.reduce((prev, current) => 
                (prev.hp > current.hp) ? prev : current
            );
        }
        
        // 🔥 CRIA O CLONE com nova imagem
        const clone = {
            x: cloneX,
            y: cloneY,
            spawnTime: Date.now(),
            detonationTime: Date.now() + data.illusionDelay,
            radius: data.illusionRadius,
            owner: this,
            target: strongestEnemy,
            speed: data.illusionSpeed,
            
            // 🔥 NOVA IMAGEM DO CLONE
            image: new Image(),
            alpha: 0.7,
            pulsePhase: 0,
            trailParticles: []
        ...
        clone.image.src = './assets_img/Clone_Jean_Grey.webp';
        clone.image.onerror = () => {
            console.warn('Clone image not found, using fallback');
            clone.image.isFallback = true;
        };
        
        this.gameManager.telepathicIllusions = this.gameManager.telepathicIllusions || [];
        this.gameManager.telepathicIllusions.push(clone);
        
        this.abilities.telepathicIllusion.cooldown = data.telepathicIllusionCooldown;
        
        if (strongestEnemy) {
            this.gameManager.showUI('Jean Grey: Clone perseguindo inimigo!', 'special');
        } else {
            this.gameManager.showUI('Jean Grey: Clone criado!', 'special');
        }
    }
    
    // ===============================
    // HABILIDADE 3: RESSURGIMENTO DA FÊNIX (NOVA)
    // ===============================
    else if (abilityNumber === 3 && this.abilities.phoenixRebirth.cooldown <= 0) {
        // Ativa modo de seleção de área
        this.gameManager.isSelectingPhoenixRebirth = true;
        this.gameManager.phoenixRebirthOwner = this;
        
        this.gameManager.showUI('Jean Grey: Clique onde a Fênix deve aterrissar!', 'ultimate');
        
        this.abilities.phoenixRebirth.cooldown = data.phoenixRebirthCooldown;
    }
}
}

export class NickFury extends Champion {
    constructor(type, x, y, id, gameManagerInstance) {
        super(type, x, y, id, gameManagerInstance);
        
        const data = Champion.championData.nickfury;
        
        // ✈️ Sistema de Quinjets
        this.abilities.quinjet = {
            cooldown: 0,
            cooldownBase: data.quinjetCooldown
        };
        
        this.activeQuinjets = [];
        
        // 👥 Sistema de Agentes
        this.abilities.agent = {
            cooldown: 0,
            cooldownBase: data.agentCooldown
        };
        
        this.activeAgents = [];
        this.agentTrails = []; // Rastros de cura
        
        // 🔫 Estado das pistolas
        this.pistolFlashLeft = false;
        this.pistolFlashRight = false;
        this.lastPistolFlash = 0;
        
        // 🛳️ AEROPORTA-AVIÕES
        this.carrierY = y;
        this.carrierTargetY = y;
        this.carrierSpeed = 50;
        this.carrierChangeTimer = 0;
        this.carrierChangeInterval = 4000;
        this.carrierMinY = 50;
        this.carrierMaxY = gameManagerInstance.canvas.height - 100;

        // Habilidade 3: Bombardeio Orbital
        this.abilities.orbitalStrike = {
            cooldown: 0,
            cooldownBase: data.orbitalStrikeCooldown
        };
        
        console.log('🧍✅ Nick Fury criado!');
    }
    ...
    update(deltaTime, enemies, champions, projectiles, effects) {
        ...
        // 🛳️ MOVIMENTO VERTICAL DO AEROPORTA-AVIÕES
        this.carrierChangeTimer += deltaTime;

        // 🛡️ ZONA DE COMANDO (PASSIVA)
        this.applyCommandZone(champions);
        
        if (this.carrierChangeTimer >= this.carrierChangeInterval) {
            this.carrierTargetY = this.carrierMinY + Math.random() * (this.carrierMaxY - this.carrierMinY);
            this.carrierChangeTimer = 0;
        }
        
        if (Math.abs(this.carrierY - this.carrierTargetY) > 2) {
            const direction = this.carrierTargetY > this.carrierY ? 1 : -1;
            this.carrierY += direction * this.carrierSpeed * (deltaTime / 1000);
            this.y = this.carrierY;
        }
        
        // ✈️ ATUALIZA QUINJETS (VOO DIRETO ESTILO IRON MAN + RETORNO)
        this.activeQuinjets = this.activeQuinjets.filter(quinjet => {
            const elapsed = Date.now() - quinjet.spawnTime;
            
            // ✅ FASE 1: VOO DE ATAQUE (primeiros 20 segundos)
            if (elapsed < data.quinjetFlightDuration - 5000) {
                // Se alvo morreu ou está muito longe, busca novo
                if (!quinjet.target || quinjet.target.hp <= 0) {
                    quinjet.target = this.findBestTarget(quinjet, enemies);
                    
                    if (!quinjet.target) {
                        // Sem alvos, começa retorno antecipado
                        quinjet.isReturning = true;
                    }
                }
                
                if (!quinjet.isReturning) {
                    // VOO DIRETO ATÉ O ALVO
                    const targetX = quinjet.target.getCenterX();
                    const targetY = quinjet.target.getCenterY();
                    
                    const distToTarget = Math.hypot(targetX - quinjet.x, targetY - quinjet.y);
                    
                    if (distToTarget > data.quinjetOrbitRadius) {
                        const angle = Math.atan2(targetY - quinjet.y, targetX - quinjet.x);
                        quinjet.x += Math.cos(angle) * data.quinjetFlightSpeed * (deltaTime / 1000);
                        quinjet.y += Math.sin(angle) * data.quinjetFlightSpeed * (deltaTime / 1000);
                        quinjet.rotation = angle;
                    } else {
                        quinjet.orbitAngle += (data.quinjetFlightSpeed / data.quinjetOrbitRadius) * (deltaTime / 1000);
                        quinjet.x = targetX + Math.cos(quinjet.orbitAngle) * data.quinjetOrbitRadius;
                        quinjet.y = targetY + Math.sin(quinjet.orbitAngle) * data.quinjetOrbitRadius;
                        quinjet.rotation = quinjet.orbitAngle + Math.PI / 2;
                    }
                    
                    // ATAQUE
                    quinjet.attackCooldown -= deltaTime;
                    
                    if (quinjet.attackCooldown <= 0 && distToTarget < data.quinjetRetargetRange) {
                        this.quinjetAttack(quinjet, quinjet.target, projectiles, effects);
                        quinjet.attackCooldown = data.quinjetAttackRate;
                    }
                }
            } 
            // ✅ FASE 2: RETORNO AO NICK FURY
            else {
                quinjet.isReturning = true;
            }
            ...
            // 🔄 LÓGICA DE RETORNO
            if (quinjet.isReturning) {
                const distToNick = Math.hypot(
                    this.getCenterX() - quinjet.x,
                    this.getCenterY() - quinjet.y
                );
                
                // Se chegou perto do Nick, remove
                if (distToNick < 50) {
                    this.gameManager.effects.push(new this.gameManager.BamfEffect(
                        quinjet.x, quinjet.y, 'cyan', 400
                    ));
                    this.gameManager.showUI('Quinjet retornou à base! ✈️', 'info');
                    return false;
                }
                
                // Voa de volta ao Nick
                const angleToNick = Math.atan2(
                    this.getCenterY() - quinjet.y,
                    this.getCenterX() - quinjet.x
                );
                
                quinjet.x += Math.cos(angleToNick) * data.quinjetFlightSpeed * (deltaTime / 1000);
                quinjet.y += Math.sin(angleToNick) * data.quinjetFlightSpeed * (deltaTime / 1000);
                quinjet.rotation = angleToNick;
            }
            
            // Trail de partículas
            if (Math.random() < 0.3) {
                quinjet.trailParticles.push({
                    x: quinjet.x - Math.cos(quinjet.rotation) * 20,
                    y: quinjet.y - Math.sin(quinjet.rotation) * 20,
                    life: 1,
                    size: 3 + Math.random() * 2,
                    vx: -Math.cos(quinjet.rotation) * 50,
                    vy: -Math.sin(quinjet.rotation) * 50
                });
            }
            
            quinjet.trailParticles = quinjet.trailParticles.filter(p => {
                p.x += p.vx * (deltaTime / 1000);
                p.y += p.vy * (deltaTime / 1000);
                p.life -= 0.025;
                return p.life > 0;
        ...
        // 👥 ATUALIZA AGENTES
        this.activeAgents = this.activeAgents.filter(agent => {
            const elapsed = Date.now() - agent.spawnTime;
            
            // Remove se tempo acabou
            if (elapsed > agent.duration) {
                this.gameManager.effects.push(new this.gameManager.BamfEffect(
                    agent.x, agent.y, 'green', 500
                ));
                this.gameManager.showUI(`${agent.name} extraído!`, 'info');
                return false;
            }
            
            // Se ainda está descendo
            if (agent.isDropping) {
                agent.dropProgress += deltaTime / agent.dropTime;
                agent.y = agent.dropStartY + (agent.dropEndY - agent.dropStartY) * agent.dropProgress;
                
                if (agent.dropProgress >= 1) {
                    agent.isDropping = false;
                    agent.y = agent.dropEndY;
                }
                
                return true;
            }
            
            // MOVIMENTO DO AGENTE EM DIREÇÃO AO ALVO
            if (!agent.target || agent.target.hp <= 0) {
                agent.target = this.findBestTarget(agent, enemies);
            }
            
            if (agent.target) {
                const distToTarget = Math.hypot(
                    agent.target.getCenterX() - agent.x,
                    agent.target.getCenterY() - agent.y
                );
                
                if (distToTarget > agent.range * 0.7) {
                    // Move em direção ao alvo
                    const angle = Math.atan2(
                        agent.target.getCenterY() - agent.y,
                        agent.target.getCenterX() - agent.x
                    ...
                    agent.x += Math.cos(angle) * data.agentMoveSpeed * (deltaTime / 1000);
                    agent.y += Math.sin(angle) * data.agentMoveSpeed * (deltaTime / 1000);
                    
                    // CRIA RASTRO DE CURA AZUL
                    if (Math.random() < 0.5) {
                        this.agentTrails.push({
                            x: agent.x,
                            y: agent.y,
                            spawnTime: Date.now(),
                            lifetime: data.agentTrailLifetime,
                            radius: data.agentTrailRadius,
                            healRate: data.agentTrailHealRate,
            ...
            if (agent.attackCooldown <= 0) {
                this.agentAttack(agent, enemies, projectiles, effects);
        ...
        // 💙 ATUALIZA RASTROS DE CURA
        this.agentTrails = this.agentTrails.filter(trail => {
            const elapsed = Date.now() - trail.spawnTime;
            
            if (elapsed > trail.lifetime) {
                return false;
            }
            
            // Cura aliados próximos
            if (Date.now() - trail.lastHealTime >= 500) { // A cada 0.5s
                champions.forEach(ally => {
                    if (ally.hp < ally.maxHp) {
                        const dist = Math.hypot(trail.x - ally.getCenterX(), trail.y - ally.getCenterY());
                        
                        if (dist < trail.radius) {
                            const healAmount = trail.healRate * 0.5;
                            ally.hp = Math.min(ally.maxHp, ally.hp + healAmount);
                ...
                trail.lastHealTime = Date.now();
            }
            
            return true;
        });
        
        // Reseta flash das pistolas
        if (Date.now() - this.lastPistolFlash > 100) {
            this.pistolFlashLeft = false;
            this.pistolFlashRight = false;
        }
    }
    ...
    findBestTarget(unit, enemies) {
        ...
        enemies.forEach(enemy => {
            const dist = Math.hypot(unit.x - enemy.getCenterX(), unit.y - enemy.getCenterY());
            
            // Prioridade: HP alto + perto
            const priority = enemy.hp - dist * 0.3;
    ...
    attack(enemies, projectiles, effects) {
        if (this.lastAttackTime <= 0) {
            ...
            const targets = enemies
                .filter(e => {
                    const dist = Math.hypot(
                        this.getCenterX() - e.getCenterX(),
                        this.getCenterY() - e.getCenterY()
                    );
                    return dist < data.alcance;
                })
                .sort((a, b) => {
            ...
            const damagePerShot = targets.length === 2 
                ? data.pistolDamage * 0.7 
                : data.pistolDamage;
            
            targets.forEach((target, index) => {
                const bullet = new NickFuryBullet(
                    this.getCenterX() + (index === 0 ? -8 : 8),
                    this.getCenterY(),
                    target.getCenterX(),
                    target.getCenterY(),
                    data.pistolSpeed,
                    damagePerShot * (1 + this.damageBoostBuff),
                    this,
                    this.gameManager
                );
                ...
                if (index === 0) {
                    this.pistolFlashLeft = true;
                } else {
                    this.pistolFlashRight = true;
                }
            });
            
            this.lastPistolFlash = Date.now();
            this.lastAttackTime = this.cooldownBase;
        }
    }
    
    // ✈️ ATAQUE DO QUINJET
    quinjetAttack(quinjet, target, projectiles, effects) {
        const data = Champion.championData.nickfury;
        
        if (!target || target.hp <= 0) return;
        
        if (Math.random() < data.quinjetMissileChance) {
            // 🚀 MÍSSIL
            const missile = new QuinjetMissile(
                quinjet.x,
                quinjet.y + 10,
                target.getCenterX(),
                target.getCenterY(),
                600,
                data.quinjetMissileDamage,
                data.quinjetMissileRadius,
                this,
                this.gameManager
            );
            ...
            effects.push(new this.gameManager.AuraFireParticleEffect(
                quinjet.x, quinjet.y, 15, 'orange', 300
            ));
        } else {
            // 🔫 RAJADA
            for (let i = 0; i < 3; i++) {
                const bullet = new QuinjetBullet(
                    quinjet.x + (Math.random() - 0.5) * 10,
                    quinjet.y + 8,
                    target.getCenterX() + (Math.random() - 0.5) * 30,
                    target.getCenterY() + (Math.random() - 0.5) * 30,
                    700,
                    data.quinjetBulletDamage,
                    this,
                    this.gameManager
                );
    ...
    // 👥 ATAQUE DO AGENTE
    agentAttack(agent, enemies, projectiles, effects) {
        if (!agent.target || agent.target.hp <= 0) return;
        
        const target = agent.target;
        const dist = Math.hypot(agent.x - target.getCenterX(), agent.y - target.getCenterY());
        
        if (dist > agent.range) return;
        
        // ATAQUES POR TIPO
        switch (agent.type) {
            case 'mariahill':
                const smgBullet = new NickFuryBullet(
                    agent.x, agent.y,
                    target.getCenterX(), target.getCenterY(),
                    900, agent.damage, this, this.gameManager
                );
                projectiles.push(smgBullet);
                break;
                
            case 'coulson':
                const rocket = new QuinjetMissile(
                    agent.x, agent.y,
                    target.getCenterX(), target.getCenterY(),
                    500, agent.damage, agent.explosionRadius,
                    this, this.gameManager
                );
            ...
            case 'agent13':
                ...
                setTimeout(() => {
                    if (target.hp > 0) {
                        target.takeDamage(agent.damage, this);
                        effects.push(new this.gameManager.TextPopEffect(
                            target.getCenterX(), target.getCenterY() - 20,
                            `SNIPER ${agent.damage}`, 'red', 800
                        ));
                    }
                }, 300);
                break;
            ...
            case 'valentina':
                effects.push(new this.gameManager.RedHulkExplosionEffect(
                    target.getCenterX(), target.getCenterY(),
                    agent.shockRadius, 500, 'rgba(100, 200, 255, 0.8)'
                ));
                
                enemies.forEach(e => {
                    const d = Math.hypot(target.getCenterX() - e.getCenterX(), target.getCenterY() - e.getCenterY());
                    if (d < agent.shockRadius) {
                        e.takeDamage(agent.damage, this);
                        e.applyStun(agent.shockDuration);
                    }
                });
                break;
            ...
            case 'mockingbird':
                if (dist < agent.range) {
                    target.takeDamage(agent.damage, this);
                    effects.push(new this.gameManager.SwordCutEffect(
                        agent.x, agent.y,
                        target.getCenterX(), target.getCenterY(),
                        'white', 10
                    ));
    ...
    // 🎯 ENCONTRA PONTO MAIS CONGESTIONADO
    findMostCongestedPoint() {
        const data = Champion.championData.nickfury;
        ...
        // Testa cada inimigo como centro
        enemies.forEach(enemy => {
            let nearbyCount = 0;
            
            enemies.forEach(other => {
                const dist = Math.hypot(
                    enemy.getCenterX() - other.getCenterX(),
                    enemy.getCenterY() - other.getCenterY()
                );
                
                if (dist < data.orbitalStrikeDetectionRadius) {
                    nearbyCount++;
                }
            });
    ...
    // 🎯 LANÇA MÍSSIL ORBITAL (DO AEROPORTA-AVIÕES) - VERSÃO COM EFEITO
    launchOrbitalMissile(targetX, targetY) {
        const data = Champion.championData.nickfury;
        
        // Cria efeito visual do míssil caindo
        this.gameManager.effects.push(new this.gameManager.OrbitalMissileEffect(
            this.getCenterX(),
            this.getCenterY(),
            targetX,
            targetY,
            1000 // 1 segundo de queda
        ));
        
        // Impacto após 1 segundo
        setTimeout(() => {
            // Flash branco intenso
            this.gameManager.effects.push(new this.gameManager.BamfEffect(
                targetX, targetY, 'white', 300
            ));
            
            // Explosão massiva
            this.gameManager.effects.push(new this.gameManager.RedHulkExplosionEffect(
                targetX, targetY, data.orbitalStrikeMissileRadius, 800, 'rgba(255, 255, 255, 0.9)'
            ));
            ...
            this.gameManager.enemies.forEach(enemy => {
                ...
                if (dist < data.orbitalStrikeMissileRadius) {
                    enemy.takeDamage(data.orbitalStrikeMissileDamage, this);
                    
                    this.gameManager.effects.push(new this.gameManager.TextPopEffect(
                        enemy.getCenterX(),
                        enemy.getCenterY() - 25,
                        `💥 ${data.orbitalStrikeMissileDamage}`,
                        'white',
                        800
                    ));
                }
            ...
            for (let i = 0; i <= 3; i++) {
                setTimeout(() => {
                    this.gameManager.effects.push(new this.gameManager.USAgentShockwaveEffect(
                        targetX, targetY, 
                        data.orbitalStrikeMissileRadius * (0.5 + i * 0.2), 
                        400
                    ));
                }, i * 120);
            }
    ...
    // 🛡️ ZONA DE COMANDO (PASSIVA)
    applyCommandZone(champions) {
        const data = Champion.championData.nickfury;
        
        champions.forEach(ally => {
            if (ally.id === this.id || ally.hp <= 0) return;
            ...
            if (dist < data.commandZoneRadius) {
                // Marca como dentro da zona
                ally.inCommandZone = true;
                ally.commandZoneDamageBonus = data.commandZoneDamageBonus;
                ally.commandZoneSpeedBonus = data.commandZoneSpeedBonus;
            } else {
                ally.inCommandZone = false;
                ally.commandZoneDamageBonus = 0;
                ally.commandZoneSpeedBonus = 0;
            }
        });
    }
    ...
    activateAbility(abilityNumber) {
        ...
        // ✈️ HABILIDADE 1: Quinjets
        if (abilityNumber === 1 && this.abilities.quinjet.cooldown <= 0) {
            for (let i = 0; i < data.quinjetCount; i++) {
                const initialTarget = this.gameManager.enemies
                    .filter(e => e.hp > 0)
                    .sort((a, b) => b.hp - a.hp)[i] || this.gameManager.enemies[0];
                
                if (!initialTarget) continue;
                
                const quinjet = {
                    id: `quinjet-${Date.now()}-${i}`,
                    x: this.getCenterX() + 30,
                    y: this.getCenterY() + (i === 0 ? -30 : 30),
                    target: initialTarget,
                    orbitAngle: Math.random() * Math.PI * 2,
                    rotation: 0,
                    spawnTime: Date.now(),
                    flightDuration: data.quinjetFlightDuration,
                    attackCooldown: 0,
                    owner: this,
                    trailParticles: []
                };
                
                this.activeQuinjets.push(quinjet);
                
                this.gameManager.effects.push(new this.gameManager.RedHulkExplosionEffect(
                    quinjet.x, quinjet.y, 50, 400, 'rgba(100, 200, 255, 0.8)'
                ));
            }
            
            this.abilities.quinjet.cooldown = data.quinjetCooldown;
            this.gameManager.showUI('Nick Fury: Quinjets em voo direto! ✈️', 'ultimate');
        }
        
        // 👥 HABILIDADE 2: Agente SHIELD
        else if (abilityNumber === 2 && this.abilities.agent.cooldown <= 0) {
            const agentTypes = ['mariahill', 'coulson', 'agent13', 'valentina', 'mockingbird'];
            const randomType = agentTypes[Math.floor(Math.random() * agentTypes.length)];
            const agentData = data.agents[randomType];
            ...
            const agent = {
                ...
                dropStartY: -50,
                dropEndY: this.getCenterY(),
                y: -50,
                dropProgress: 0,
                dropTime: data.agentDropTime,
                isDropping: true,
                spawnTime: Date.now(),
                duration: data.agentDuration,
                attackCooldown: 0,
                target: null,
                owner: this,
                image: new Image()
            ...
            agent.image.src = agentData.image;
            agent.image.onerror = () => { agent.image.isFallback = true; };
            ...
            this.abilities.agent.cooldown = data.agentCooldown;
            this.gameManager.showUI(`${agentData.name} entrando em campo!`, 'ultimate');
        }

        // 🎯 HABILIDADE 3: Bombardeio Orbital
        else if (abilityNumber === 3 && this.abilities.orbitalStrike.cooldown <= 0) {
            const targetPoint = this.findMostCongestedPoint();
            
            if (!targetPoint) {
                this.gameManager.showUI('Sem inimigos para bombardear!', 'warning');
                return;
            }
            
            // Marca o alvo com laser tecnológico
            this.gameManager.effects.push(new TargetLaserEffect(
                targetPoint.x, targetPoint.y, 2000, 'cyan'
            ));
            
            // Som de rádio (você pode adicionar depois)
            this.gameManager.showUI('Nick Fury: Bombardeio orbital autorizado! 🎯', 'ultimate');
            
            // Lança mísseis em sequência
            for (let i = 0; i < data.orbitalStrikeMissileCount; i++) {
                setTimeout(() => {
                    const offsetX = (Math.random() - 0.5) * 80;
                    const offsetY = (Math.random() - 0.5) * 80;
                    
                    this.launchOrbitalMissile(
                        targetPoint.x + offsetX,
                        targetPoint.y + offsetY
                    );
                }, i * data.orbitalStrikeMissileDelay);
            }
            
            this.abilities.orbitalStrike.cooldown = data.orbitalStrikeCooldown;
        }
    }
    
    // 🎨 VISUAL CUSTOMIZADO
    draw(ctx, isSelected, mjolnirImage, capShieldImage, usagentShieldImage, wandaIllusionImage, displayRangeOverride = null) {
        // ✅ CORREÇÃO: Define 'data' dentro do método draw
        const data = Champion.championData.nickfury;
        const centerX = this.getCenterX();
        const centerY = this.getCenterY();
        ...
        // ===============================
        // 🛳️ HELICARRIER SHIELD - DESIGN COMPLETO
        // ===============================
        
        const carrierWidth = 180;
        const carrierHeight = 70;
        const carrierX = this.x - 65;
        const carrierY = this.y;
        
        // ===== SOMBRA REALISTA =====
        ctx.fillStyle = 'rgba(0, 0, 0, 0.4)';
        ctx.fillRect(carrierX - 5, carrierY + carrierHeight + 5, carrierWidth + 10, 20);
        
        // ===== CASCO PRINCIPAL =====
        // Cor base: cinza escuro metálico
        ctx.fillStyle = '#2B2E34';
        ctx.fillRect(carrierX, carrierY + 15, carrierWidth, carrierHeight - 15);
        
        // Nariz pontudo
        ctx.fillStyle = '#1F2226';
        ctx.beginPath();
        ctx.moveTo(carrierX + carrierWidth, carrierY + 20);
        ctx.lineTo(carrierX + carrierWidth + 15, carrierY + 35);
        ctx.lineTo(carrierX + carrierWidth, carrierY + 50);
        ctx.closePath();
        ctx.fill();
        
        // ===== PLACAS METÁLICAS (TEXTURA) =====
        ctx.strokeStyle = '#1A1C1F';
        ctx.lineWidth = 1;
        
        // Linhas horizontais
        for (let i = 1; i < 4; i++) {
            const y = carrierY + 15 + (carrierHeight - 15) * (i / 4);
            ctx.beginPath();
            ctx.moveTo(carrierX, y);
            ctx.lineTo(carrierX + carrierWidth, y);
            ctx.stroke();
        }
        
        // Linhas verticais
        for (let i = 1; i < 8; i++) {
            const x = carrierX + carrierWidth * (i / 8);
            ctx.beginPath();
            ctx.moveTo(x, carrierY + 15);
            ctx.lineTo(x, carrierY + carrierHeight);
            ctx.stroke();
        }
        
        // ===== CONVÉS SUPERIOR (PISTA) =====
        ctx.fillStyle = '#4A4F57';
        ctx.fillRect(carrierX, carrierY + 10, carrierWidth, 8);
        
        // Pista de pouso - linhas brancas/amarelas
        ctx.strokeStyle = 'rgba(255, 200, 0, 0.7)';
        ctx.lineWidth = 2;
        ctx.setLineDash([12, 8]);
        ctx.beginPath();
        ctx.moveTo(carrierX + 10, carrierY + 14);
        ctx.lineTo(carrierX + carrierWidth - 10, carrierY + 14);
        ctx.stroke();
        ctx.setLineDash([]);
        
        // Logo SHIELD (centro do convés)
        ctx.save();
        ctx.translate(carrierX + carrierWidth / 2, carrierY + 14);
        ...
        // ===== TORRE DE COMANDO (ILHA - LADO DIREITO) =====
        const towerX = carrierX + carrierWidth - 45;
        const towerY = carrierY - 10;
        
        // Base da torre
        ctx.fillStyle = '#2B2E34';
        ctx.fillRect(towerX, towerY + 10, 35, 15);
        
        // Bloco principal
        ctx.fillStyle = '#1F2226';
        ctx.fillRect(towerX + 3, towerY, 29, 20);
        
        // Topo da torre
        ctx.fillStyle = '#2B2E34';
        ctx.fillRect(towerX + 5, towerY - 5, 25, 6);
        
        // Janelas HUD (azul-claro)
        const windowColor = 'rgba(100, 200, 255, 0.8)';
        ctx.fillStyle = windowColor;
        
        // Linha de janelas
        for (let i = 0; i < 4; i++) {
            ctx.fillRect(towerX + 7 + i * 6, towerY + 3, 4, 6);
        }
        
        // Janelas superiores
        ctx.fillRect(towerX + 10, towerY + 12, 4, 5);
        ctx.fillRect(towerX + 17, towerY + 12, 4, 5);
        ...
        ctx.lineTo(towerX + 8, towerY - 12);
        ctx.stroke();
        
        // Disco radar
        ctx.fillStyle = '#4A4F57';
        ctx.beginPath();
        ctx.arc(towerX + 8, towerY - 12, 3, 0, Math.PI * 2);
        ctx.fill();
        
        // Antena direita
        ctx.beginPath();
        ctx.moveTo(towerX + 27, towerY - 5);
        ctx.lineTo(towerX + 27, towerY - 15);
        ctx.stroke();
        
        ctx.fillStyle = '#4A4F57';
        ctx.fillRect(towerX + 25, towerY - 17, 4, 2);
        
        // ===== MOTORES (4 GRANDES) =====
        const motorData = [
            { x: carrierX + 15, y: carrierY + carrierHeight - 5 },
            { x: carrierX + 45, y: carrierY + carrierHeight - 5 },
            { x: carrierX + carrierWidth - 45, y: carrierY + carrierHeight - 5 },
            { x: carrierX + carrierWidth - 15, y: carrierY + carrierHeight - 5 }
        ];
        
        motorData.forEach(motor => {
            // Carcaça do motor (hexagonal)
            ctx.fillStyle = '#1A1C1F';
            ctx.beginPath();
            for (let i = 0; i < 6; i++) {
                const angle = (Math.PI / 3) * i;
                const mx = motor.x + Math.cos(angle) * 10;
                const my = motor.y + Math.sin(angle) * 10;
            ...
            // Núcleo azul (glow suave)
            const motorGlow = ctx.createRadialGradient(motor.x, motor.y, 0, motor.x, motor.y, 8);
            motorGlow.addColorStop(0, 'rgba(100, 200, 255, 1)');
            motorGlow.addColorStop(0.5, 'rgba(100, 200, 255, 0.6)');
            motorGlow.addColorStop(1, 'rgba(100, 200, 255, 0)');
            
            ctx.fillStyle = motorGlow;
            ctx.beginPath();
            ctx.arc(motor.x, motor.y, 8, 0, Math.PI * 2);
            ctx.fill();
            
            // Centro brilhante
            ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
            ctx.beginPath();
            ctx.arc(motor.x, motor.y, 3, 0, Math.PI * 2);
            ctx.fill();
            
            // Partículas leves (se em movimento)
            if (Math.random() < 0.3) {
                const particleX = motor.x + (Math.random() - 0.5) * 15;
                const particleY = motor.y + Math.random() * 8;
        ...
        // ===== ARMAMENTO DISCRETO =====
        // Torres laterais pequenas
        const turretPositions = [
            { x: carrierX + 30, y: carrierY + 25 },
            { x: carrierX + carrierWidth - 30, y: carrierY + 25 }
        ];
        
        turretPositions.forEach(turret => {
            // Base
            ctx.fillStyle = '#2B2E34';
            ctx.fillRect(turret.x - 4, turret.y - 2, 8, 6);
            
            // Canhão
            ctx.fillStyle = '#1F2226';
            ctx.fillRect(turret.x - 1, turret.y - 6, 2, 8);
            
            // Ponta vermelha (laser)
            ctx.fillStyle = 'rgba(255, 50, 50, 0.7)';
            ctx.fillRect(turret.x - 0.5, turret.y - 7, 1, 2);
        });
        
        // Mísseis embutidos (apenas contorno)
        ctx.strokeStyle = '#1A1C1F';
        ctx.lineWidth = 1;
        
        for (let i = 0; i < 6; i++) {
            const mx = carrierX + 60 + i * 15;
            ctx.strokeRect(mx, carrierY + 40, 8, 3);
        }
        ...
        const lightPositions = [
            ...
            // Traseira (azul)
            { x: carrierX + 5, y: carrierY + 60, color: 'rgba(100, 200, 255, 0.9)' },
            { x: carrierX + 5, y: carrierY + 30, color: 'rgba(100, 200, 255, 0.9)' }
        ];
        
        lightPositions.forEach(light => {
            const blinkAlpha = Math.sin(Date.now() / 400 + light.x) * 0.3 + 0.7;
            ctx.fillStyle = light.color.replace(/[\d.]+\)/, `${blinkAlpha})`);
            ctx.beginPath();
        ...
        // ===== LUZES DE NAVEGAÇÃO (PISCANTES) =====
        const navLights = [
            { x: carrierX + carrierWidth + 10, y: carrierY + 35, color: 'red' }, // Ponta direita
            { x: carrierX + 5, y: carrierY + 35, color: 'green' }, // Ponta esquerda
        ];
        
        const navBlink = Math.sin(Date.now() / 300) * 0.5 + 0.5;
        navLights.forEach(nav => {
            ctx.fillStyle = nav.color === 'red' 
                ? `rgba(255, 0, 0, ${navBlink})` 
                : `rgba(0, 255, 0, ${navBlink})`;
            ctx.beginPath();
            ctx.arc(nav.x, nav.y, 3, 0, Math.PI * 2);
            ctx.fill();
        });
        
        // ===== DETALHES FINAIS =====
        // Marca "S.H.I.E.L.D." discreta
        ctx.fillStyle = 'rgba(100, 150, 200, 0.4)';
        ctx.font = 'bold 6px Arial';
        ctx.textAlign = 'left';
        ctx.fillText('S.H.I.E.L.D.', carrierX + 10, carrierY + 70);
        
        ctx.restore();
        
        // ===== NICK FURY (EM CIMA DO CONVÉS) =====
        super.draw(ctx, isSelected, mjolnirImage, capShieldImage, usagentShieldImage, wandaIllusionImage, displayRangeOverride);
        
        // 🔫 FLASH DAS PISTOLAS
        if (this.pistolFlashLeft) {
            ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
        ...
        // 💙 DESENHA RASTROS DE CURA AZUL
        this.agentTrails.forEach(trail => {
            const elapsed = Date.now() - trail.spawnTime;
            const lifePercent = 1 - (elapsed / trail.lifetime);
            
            const trailGradient = ctx.createRadialGradient(
                trail.x, trail.y, 0,
                trail.x, trail.y, trail.radius
            );
            trailGradient.addColorStop(0, `rgba(100, 200, 255, ${lifePercent * 0.4})`);
            trailGradient.addColorStop(0.7, `rgba(100, 200, 255, ${lifePercent * 0.2})`);
            ...
            if (Math.random() < 0.2) {
                const angle = Math.random() * Math.PI * 2;
                const dist = Math.random() * trail.radius;
                
                ctx.fillStyle = `rgba(150, 220, 255, ${lifePercent})`;
                ctx.beginPath();
                ctx.arc(
                    trail.x + Math.cos(angle) * dist,
                    trail.y + Math.sin(angle) * dist,
                    2 + Math.random() * 2,
                    0, Math.PI * 2
                );
                ctx.fill();
        ...
        // ✈️ DESENHA QUINJETS (código anterior mantido)
        this.activeQuinjets.forEach(quinjet => {
            ctx.save();
            
            quinjet.trailParticles.forEach(p => {
                const particleGradient = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.size);
                particleGradient.addColorStop(0, `rgba(150, 200, 255, ${p.life * 0.8})`);
                particleGradient.addColorStop(1, `rgba(100, 150, 200, 0)`);
            ...
            if (quinjet.target && quinjet.target.hp > 0) {
                ...
                ctx.setLineDash([5, 5]);
                ctx.beginPath();
                ctx.moveTo(quinjet.x, quinjet.y);
                ctx.lineTo(quinjet.target.getCenterX(), quinjet.target.getCenterY());
                ctx.stroke();
                ctx.setLineDash([]);
            }
            
            ctx.save();
            ctx.translate(quinjet.x, quinjet.y);
            ctx.rotate(quinjet.rotation);
            
            ctx.fillStyle = 'rgba(0, 0, 0, 0.2)';
            ctx.fillRect(-22, 5, 44, 18);
            
            ctx.fillStyle = 'rgba(70, 75, 80, 0.95)';
            ...
            const engineGlow = ctx.createRadialGradient(-20, 0, 0, -20, 0, 8);
            engineGlow.addColorStop(0, 'rgba(255, 150, 0, 0.9)');
            ...
            const lightBlink = Math.sin(Date.now() / 150) * 0.5 + 0.5;
            ctx.fillStyle = `rgba(0, 255, 255, ${lightBlink})`;
            ctx.beginPath();
        ...
        // 👥 DESENHA AGENTES (código anterior mantido)
        this.activeAgents.forEach(agent => {
            ...
            if (agent.isDropping) {
                ...
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.moveTo(agent.x, -20);
                ctx.lineTo(agent.x, agent.y);
                ctx.stroke();
                
                ctx.fillStyle = 'rgba(80, 80, 80, 0.9)';
                ctx.fillRect(agent.x - 15, -30, 30, 10);
                
                ctx.save();
                ctx.translate(agent.x, -25);
            ...
            if (agent.image && agent.image.complete && !agent.image.isFallback) {
                ctx.drawImage(agent.image, agent.x - 20, agent.y - 20, 40, 40);
            } else {
                ctx.fillStyle = 'rgba(50, 100, 50, 0.9)';
                ctx.fillRect(agent.x - 15, agent.y - 20, 30, 40);
        ...
        // 🛡️ ZONA DE COMANDO (VISUAL)
        const zoneRadius = data.commandZoneRadius;
        const zonePulse = Math.sin(Date.now() / 500) * 10;

        // Círculo holográfico
        const zoneGradient = ctx.createRadialGradient(
            centerX, centerY, 0,
            centerX, centerY, zoneRadius + zonePulse
        );
        zoneGradient.addColorStop(0, 'rgba(100, 200, 255, 0)');
        zoneGradient.addColorStop(0.7, 'rgba(100, 200, 255, 0.15)');
        zoneGradient.addColorStop(1, 'rgba(100, 200, 255, 0)');

        ctx.fillStyle = zoneGradient;
        ctx.beginPath();
        ctx.arc(centerX, centerY, zoneRadius + zonePulse, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Símbolo SHIELD no centro (translúcido)
        ctx.save();
        ctx.globalAlpha = 0.3;
        ctx.translate(centerX, centerY);

        // Desenha símbolo SHIELD (círculo com águia simplificada)
...
Champion.championData = {
   ...
   gambit: {
    ...
    jeangrey: {
    icon: './assets_img/JeanGrey.webp',
    imagePath: './assets_img/JeanGrey.webp',
    poder: 'Chamas Cósmicas aplicam Faíscas. Com 3 Faíscas, explode. Rajada Telecinética: 3 explosões (stun + slow). Ilusão Telepática: recua e deixa clone explosivo.',
    custo: 400,
    dano: 15,
    hp: 450,
    alcance: 200,
    ...
    explosionDamage: 12,
    explosionRadius: 120,
    explosionCooldown: 2000,

    // ⭐ PODERES PSÍQUICOS AUTOMÁTICOS
    // Onda Psíquica (Rosa)
    psychicWaveRadius: 180,
    psychicWaveDamage: 40,
    psychicWaveStunChance: 0.4, // 40%
    psychicWaveStunDuration: 1500,
    psychicWavePushForce: 50,
    
    // Vórtice de Impacto (Azul)
    vortexRange: 200,
    vortexDamage: 60,
    vortexStunDuration: 2000,
    
 
    // ===============================
    // HABILIDADE 1: RAJADA DE TELECINESE
    // ===============================
    telekineticBarrageCooldown: 20000, // 20 segundos
    telekineticBarrageRadius: 100, // Raio da área marcada
    telekineticBarrageDamage: 20, // Dano por explosão
    telekineticBarrageExplosions: 3, // Número de explosões
    telekineticBarrageDelay: 600, // Delay entre explosões (ms)
    telekineticBarrageStunDuration: 1500, // Stun da 1ª explosão
    telekineticBarrageSlowFactor: 0.5, // 50% de slow
    telekineticBarrageSlowDuration: 3000, // 3s de slow (2ª e 3ª explosões)
    
     // HABILIDADE 2: ILUSÃO TELEPÁTICA (MELHORADA)
    telepathicIllusionCooldown: 15000,
    illusionMoveDistance: 120,
    illusionDelay: 4000, // 4s até explodir
    illusionRadius: 80,
    illusionDamage: 50,
    illusionSpeed: 200, // 🔥 NOVO: Velocidade do clone
    illusionSearchRadius: 300, // 🔥 NOVO: Alcance de busca
    
    // 🔥 HABILIDADE 3: RESSURGIMENTO DA FÊNIX (NOVA)
    phoenixRebirthCooldown: 60000, // 60 segundos
    phoenixRebirthDescent: 2000, // 2.5s de descida
    phoenixRebirthRadius: 200, // Raio gigante
    phoenixRebirthDamage: 150, // Dano massivo
    phoenixRebirthKillThreshold: 0.3, // Mata inimigos abaixo de 30% HP
...
/*             // ⭐ NOVO: Sistema de Regeneração
    ...
    //Futuro Big Bad
    nickfury: {
        icon: './assets_img/Nick_Fury.webp',
        imagePath: './assets_img/Nick_Fury.webp',
        poder: 'Comandante SHIELD. Dispara com duas pistolas simultaneamente em até 2 alvos. Lança Quinjets que perseguem e orbitam inimigos. Chama agentes SHIELD para suporte tático.',
        custo: 480,
        hp: 350,
        dano: 18,
        alcance: 200,
        ...
        pistolRange: 200,
        pistolSpeed: 800,
        pistolColor: 'rgba(100, 200, 255, 0.9)',

        // 🛡️ PASSIVA: Zona de Comando
        commandZoneRadius: 200,
        commandZoneDamageBonus: 0.1, // +10% dano
        commandZoneSpeedBonus: 0.1, // +10% velocidade
            
        // ✈️ Quinjets Caça (Habilidade 1) - ESTILO IRON MAN
        quinjetCooldown: 28000,
        quinjetCount: 2,
        quinjetFlightSpeed: 400, // Velocidade de voo direto
        quinjetFlightDuration: 25000, // 25 segundos voando
        quinjetAttackRate: 600,
        quinjetBulletDamage: 12,
        quinjetMissileDamage: 25,
        quinjetMissileRadius: 60,
        quinjetMissileChance: 0.3,
        quinjetRetargetRange: 400, // Distância para buscar novo alvo
        quinjetOrbitRadius: 100, // Distância de órbita ao redor do alvo
        
        // 👥 Agentes SHIELD (Habilidade 2)
        agentCooldown: 25000,
        agentDuration: 10000,
        agentDropTime: 1500,
        agentMoveSpeed: 80, // Velocidade de caminhada
        
        // Rastro de Cura Azul
        agentTrailHealRate: 5, // HP/s
        agentTrailRadius: 60, // Raio do rastro
        agentTrailLifetime: 2000, // Duração do rastro (2s)
        ...
        agents: {
            mariahill: {
                ...
                weapon: 'SMG',
                damage: 8,
                attackSpeed: 200, // Tiro rápido
                range: 180,
                image: './assets_img/maria_hill.webp'
            },
            coulson: {
                name: 'Agente Coulson',
                weapon: 'Bazuca',
                damage: 60,
                attackSpeed: 2000, // Recarga alta
                range: 250,
                explosionRadius: 70,
                image: './assets_img/coulson.webp'
            },
            agent13: {
                name: 'Agente 13',
                weapon: 'Sniper',
                damage: 50,
                attackSpeed: 1500,
                range: 350, // Longo alcance
                image: './assets_img/agent13.webp'
            },
            valentina: {
                name: 'Valentina',
                weapon: 'Choque',
                ...
                shockDuration: 2000,
                shockRadius: 80,
                image: './assets_img/valentina.webp'
            },
            mockingbird: {
                name: 'Mockingbird',
                weapon: 'Bastões',
                damage: 45,
                attackSpeed: 400, // Ataque rápido corpo a corpo
                range: 80,
                image: './assets_img/mockingbird.webp'
            }
        },

        // 🎯 HABILIDADE 3: Bombardeio Orbital
        orbitalStrikeCooldown: 40000, // 40 segundos
        orbitalStrikeMissileCount: 8, // Número de mísseis
        orbitalStrikeMissileDelay: 300, // Delay entre mísseis (ms)
        orbitalStrikeMissileDamage: 80, // Dano por míssil
        orbitalStrikeMissileRadius: 100, // Raio de impacto
        orbitalStrikeDetectionRadius: 150, // Raio para detectar "ponto congestionado"
